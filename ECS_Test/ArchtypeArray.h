...
private:
	...
	{
		static const size_t DEFAULT_CHUNK_SIZE = 2 * 1024;//2kb
	};
...
public:
	ArchtypeArray(const Archtype& archType, uint32_t archTypeIndex)
	...
	const Archtype& GetArchType() const
	{
		return m_archType;
	}
...
private:
	...
	template<typename Arg, typename ...ComType>
		requires is_component_ref<Arg>::value || is_entity<Arg>::value || is_sort_key<Arg>::value
...
public:
	...
	template<typename Callable, typename... ComType>
		requires  (is_component<ComType>::value && ...)
	...
	public:
		const ArchtypeArray* archType;
		component_offset_t<CompType...> m_componentOffsets;

		ForwardAccessor(const ArchtypeArray& archTypeArray)
			: archType(&archTypeArray),
			m_componentOffsets(archTypeArray.GetComponentOffset<AccessType...>())
		{

		}


		ForwardAccessor(const ArchtypeArray& archTypeArray, const component_offset_t<CompType...>& componentOffsets)
			: archType(&archTypeArray),
		...
		private:
			...
			constexpr const ArchtypeArray* archtype_array() const
			{
